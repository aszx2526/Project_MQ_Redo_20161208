...
using UnityEngine;
public class gDefineb_MainSceneCamera
{
...
public class onMainSceneCameraForFingerTouchControll : MonoBehaviour {
    ...
    void MouseInput()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            gDefineb_MainSceneCamera.Direction mDirection = HandDirection(m_screenPos, pos);
    ...
    void MobileInput()
    {
        ...
        if (Input.touchCount == 1)
        {
            ...
            else if (Input.touches[0].phase == TouchPhase.Moved)
            {
                Debug.Log("Moved");
                //移動攝影機
                myBigMap.GetComponent<RectTransform>().Translate(new Vector3(-Input.touches[0].deltaPosition.x * Time.deltaTime, -Input.touches[0].deltaPosition.y * Time.deltaTime, 0));
    ...
    gDefineb_MainSceneCamera.Direction mDirection;
    gDefineb_MainSceneCamera.Direction HandDirection(Vector2 StartPos, Vector2 EndPos)
    {
        ...
        if (Mathf.Abs(StartPos.x - EndPos.x) > Mathf.Abs(StartPos.y - EndPos.y))
        {
            if (Vector2.Distance(StartPos, EndPos) > 50 && StartPos.x > EndPos.x)
            {
                ...
                print("手指向左滑動");
                mDirection = gDefineb_MainSceneCamera.Direction.Left;
            }
            else if (Vector2.Distance(StartPos, EndPos) > 50 && StartPos.x < EndPos.x)
            {
                ...
                print("手指向右滑動");
                mDirection = gDefineb_MainSceneCamera.Direction.Right;
            }
        }
        else {
            ...
            {
                //手指向下滑動
                mDirection = gDefineb_MainSceneCamera.Direction.Down;
            }
            //else if (Vector2.Distance(m_screenPos, EndPos) > 1 && m_screenPos.y < EndPos.y)
            else {
                //手指向上滑動

                mDirection = gDefineb_MainSceneCamera.Direction.Up;
            }
